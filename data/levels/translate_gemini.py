...
2. **תרגום והחלפה (עיקרי):** עבור כל שדה טקסט באנגלית שזוהה:
...
3. **שימור והתאמה של טקסט עברי (משני ותנאי):** **ככלל, כל הטקסט בשפה העברית בקובץ ה-JSON *חייב להישאר ללא שינוי*.** עם זאת, **במקרים חריגים בלבד**, אם תרגום ישיר של הטקסט האנגלי לשפה ה{target_language_iw} יוצר **חוסר התאמה משמעותי או חוסר בהירות בתוכן *ביחס לטקסט העברי הקיים***, עליך לבצע **התאמה מינימלית** גם בטקסט העברי.
    * **מטרת ההתאמה בעברית:**  לוודא שהטקסט העברי **עדיין רלוונטי, מדויק ומתאים *למשמעות החדשה של הטקסט המתורגם לשפה ה{target_language_iw}***.
    * **סוגי התאמות אפשריות בעברית:**  שינוי מילה בודדת, ניסוח מחדש קל של משפט, או הבהרה קצרה - **רק במידה והדבר חיוני להבנת התוכן בשפה ה{target_language_iw}**.
    * **הימנע משינויים נרחבים בעברית:**  התאמות בעברית צריכות להיות **מצומצמות ומוגבלות למינימום ההכרחי** כדי לשמור על עקביות עם התוכן המקורי ככל הניתן.
4. **שימור מבנה JSON:** **המבנה של קובץ ה-JSON המתורגם חייב להיות *זהה לחלוטין* למבנה של קובץ ה-JSON המקורי.**  אותם שדות, אותה היררכיה, רק התוכן בשדות הטקסט האנגלי (ובמקרים חריגים, גם העברי) יוחלף.

**פלט:**

עליך להחזיר **קובץ JSON מתורגם *מלא***, המכיל את כל התוכן המתורגם והמותאם, עטוף בתוך תגי קוד Markdown ```json ... ```.

**דגשים:**

* **תרגום מדויק ואיכותי לשפה ה{target_language_iw}.**
* **החלפה ישירה של הטקסט האנגלי (בשדות הקיימים).**
* **שימור מבנה JSON.**
* **שימור טקסט עברי ככלל, והתאמה מינימלית רק במקרה של חוסר התאמה משמעותי לאחר תרגום האנגלית.**
* **התאמות בעברית צריכות להיות מצומצמות וממוקדות בשיפור העקביות והבהירות של התוכן בשפה ה{target_language_iw}**.

אני מצפה לקבל קובץ JSON מתורגם ומותאם מלא, במבנה זהה למקור, עם החלפת הטקסט האנגלי בלבד (ובמקרים חריגים, התאמות מינימליות בעברית) לשפה ה{target_language_iw}.
"""

QUALITY_CHECK_INST_TEMPLATE = """
**תפקידך:** לבדוק קבצי JSON המכילים אוצר מילים ב{target_language_iw}, תרגומים לעברית ומשפטי דוגמה בשתי השפות.

**משימה:**  עליך לבצע בדיקה **יסודית** של התוכן כדי לוודא **דיוק ניסוח ותרגום** בשתי השפות - {target_language_iw} ועברית.
...
* **מיקוד בדיקה:**  חפש משפטים שבהם יש **טעויות ניסוח ברורות**, **תרגום לא מדויק** או **שימוש בשפה שאינו תקני או מקובל** ב{target_language_iw} או בעברית.
...
- המשפט המקורי ({target_language_iw}): [הכנס משפט מקורי כאן]
  - המשפט המתורגם: [הכנס משפט מתורגם כאן]
  - סוג הבעיה: [הכנס הסבר ברור של סוג הבעיה]
  - משפט חלופי תקין ({target_language_iw}): [הכנס משפט חלופי תקין כאן]
...
def perform_quality_check(file_path, source_language_iw, target_language_iw, source_lang_code, target_lang_code):
    ...
    QUALITY_CHECK_SYS_INST = QUALITY_CHECK_INST_TEMPLATE.format(target_language_iw=target_language_iw)
