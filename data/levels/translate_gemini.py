...
**פורמט פלט:**
* אם נמצאו שגיאות, הצג **רשימה ממוקדת וברורה** של **כל המשפטים הדורשים תיקון**.
* עבור כל משפט בעייתי, הצג:
    * **את המשפט המקורי** בשפת המקור ({source_language_iw} או עברית).
    * **את המשפט המתורגם**.
    * **הסבר ברור של סוג הבעיה** (למשל: טעות תרגום, ניסוח לא תקני בעברית, טעות דקדוק ב{source_language_iw}).
    * **משפט חלופי תקין ומנוסח היטב** בשפה שבה נמצאה הבעיה.
* אם לא נמצאו שגיאות, החזר את המחרוזת: "NO_ERRORS_FOUND"
